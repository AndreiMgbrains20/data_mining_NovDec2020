...
from ..items import InstaSub, InstaFol, InstaUser


class HandshakeSpider(scrapy.Spider):
    db_type = 'MONGO'
    name = 'handshake'
    ...
    def __init__(self, login, password, users_list, *args, **kwargs):
        ...
        self.limit_follow = 650
        super(HandshakeSpider, self).__init__(*args, **kwargs)
    ...
    def get_follow_item(self, user_data, follow_user_data, response):
        for user in  follow_user_data:
            ...
            if self.stop != 'stop':
                yield response.follow(f"/{user['node']['username']}", callback=self.user_parse,cb_kwargs={'user_data': udf})
    ...
    def get_subscr_item(self, user_data, subscr_user_data, response):
        for user in subscr_user_data:
            ...
            if self.stop != 'stop':
                yield response.follow(f"/{user['node']['username']}", callback=self.user_parse, cb_kwargs={'user_data': uds})